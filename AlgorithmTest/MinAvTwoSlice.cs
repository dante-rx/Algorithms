...
namespace AlgorithmTest
{
    ...
    class MinAvTwoSlice
    {
        public int solution(int[] A)
        {
            ...
            double minAverageGlobal = ((double)A[0] + (double)A[1]) / 2;
            double minAverageLocal = minAverageGlobal;

            for (int i=2;i<A.Length;i++)
            {
                var tempAverage = (minAverageLocal * (i-minIndexLocal) + (double)A[i] ) / (i - minIndexLocal +1);

                var currentAverage = ((double)A[i-1] + (double)A[i]) / 2;

                if (currentAverage < tempAverage)
                {
                    minIndexLocal = i-1;
                    minAverageLocal = currentAverage;
                }
                else
                {
                    minAverageLocal = tempAverage;
                }

                if (minAverageLocal<minAverageGlobal)
                {
                    minIndexGlobal = minIndexLocal;
                    minAverageGlobal = minAverageLocal;
                }
            }
